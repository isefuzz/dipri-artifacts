...
if __name__ == '__main__':
    ...
    for benchmark in plot_dict:
        ...
        # Make table
        last_percent = extract_last_percent(bench_dict)
        last_percent['map_size'] = size_dict[benchmark]
        ave_percent_dict[benchmark] = last_percent
        last_rnd_ave = extract_last_rnd(bench_dict)
        last_rnd_ave['map_size'] = size_dict[benchmark]
        ave_rnd_dict[benchmark] = last_rnd_ave
    # Build statistical dfs
    percent_std_df = pd.DataFrame(ave_percent_std_dict)
    percent_df = pd.DataFrame(ave_percent_dict)
    rnd_df = pd.DataFrame(ave_rnd_dict)
    # Output to directory
    print('[LOG] Write overhead data to local:', out_dir)
    csv_path = os.path.join(out_dir, 'overhead_std.csv')
    csv_T_path = os.path.join(out_dir, 'overhead_std_T.csv')
    percent_std_df.to_csv(csv_path)
    percent_std_df.T.to_csv(csv_T_path)
    csv_path = os.path.join(out_dir, 'overhead.csv')
    csv_T_path = os.path.join(out_dir, 'overhead_T.csv')
    percent_df.to_csv(csv_path)
    percent_df.T.to_csv(csv_T_path)
    csv_path = os.path.join(out_dir, 'rnd.csv')
    csv_T_path = os.path.join(out_dir, 'rnd_T.csv')
    rnd_df.to_csv(csv_path)
    rnd_df.T.to_csv(csv_T_path)

    # Build comb and output
    overhead_rnd_comb_dict = dict()
    for fuzzer in ave_percent_dict:
        overhead_rnd_comb_dict[f'{fuzzer}-percent'] = ave_percent_dict[fuzzer]
        overhead_rnd_comb_dict[f'{fuzzer}-rnd'] = ave_rnd_dict[fuzzer]
    comb_df = pd.DataFrame(overhead_rnd_comb_dict).round(3)
    csv_path = os.path.join(out_dir, 'overhead+rnd.csv')
    csv_T_path = os.path.join(out_dir, 'overhead+rnd_T.csv')
    comb_df.to_csv(csv_path)
    comb_df.T.to_csv(csv_T_path)
